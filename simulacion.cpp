...
const int WALL_HEIGHT = 28;

// ============================================================
// CAMERA TUNING
// ============================================================
const int CAMERA_SWITCH_MS = 10000;
...
vector<vector<float>> buildZOffsets(const Mat &img, float multiplier) {
    ...
    for(int y = 0; y < h; ++y) {
        for(int x = 0; x < w; ++x) {
            int r = img.at<Vec3b>(y, x)[2]; // only R channel
...
struct Camera2D {
                                 ...
                                 long long lastPlayerInputMs)
    {
        ...
        if(nowRealMs - lastSwitchRealMs >= CAMERA_SWITCH_MS) {
            focusedNpc = (focusedNpc + 1) % (int)agents.size();
            lastSwitchRealMs = nowRealMs;
        }
...
// ============================================================
Point2f isoWorldPoint(float row, float col, float zOffset) {
    float sx = (col - row) * (TILE_W * 0.5f);
    float sy = (col + row) * (TILE_H * 0.5f) - zOffset;
...
Point2f tileWorldCenter(float row, float col, const vector<vector<float>> &zmap) {
    float z = sampleZBilinear(zmap, row, col);
    return isoWorldPoint(row, col, z);
}
...
{
    float z = sampleZBilinear(zmap, row, col);
    Point2f c = isoWorldPoint(row, col, z);

    float tw = TILE_W * 0.5f;
    float th = TILE_H * 0.5f;

    Point2f topP    = Point2f(c.x,       c.y - th);
    Point2f rightP  = Point2f(c.x + tw,  c.y);
    Point2f bottomP = Point2f(c.x,       c.y + th);
    Point2f leftP   = Point2f(c.x - tw,  c.y);
                   ...
                   const vector<vector<float>> &zmap)
{
    ...
    drawIsoPrism(img, (float)row, (float)col, FLOOR_THICKNESS, topC, cam, camCenter, zmap);

    Point base = cameraProject(tileWorldCenter((float)row, (float)col, zmap), cam, camCenter);
                  ...
                  const vector<vector<float>> &zmap)
{
    ...
    drawIsoPrism(img, (float)row, (float)col, WALL_HEIGHT, wallTop, cam, camCenter, zmap);
}
...
void drawHumanDirectionalAt(Mat &img,
                            ...
                            const vector<vector<float>> &zmap)
{
    Point2f world = tileWorldCenter(renderCell.y, renderCell.x, zmap);
    ...
    Point2f forward(cos(facing), sin(facing));
                ...
                const vector<vector<float>> &zmap)
{
    ...
    Point2f world = tileWorldCenter(p.renderCell().y, p.renderCell().x, zmap);
    Point base = cameraProject(world, cam, camCenter);
               ...
               const Camera2D &cam)
{
    ...
    Point2f camCenterWorld = tileWorldCenter(cam.centerY, cam.centerX, zmap);
    ...
    for(int sum = 0; sum <= H + W - 2; ++sum) {
        for(int row = 0; row < H; ++row) {
            ...
            if(t == WALL) drawWallTile(frame, row, col, cam, camCenterWorld, zmap);
            else drawFloorTile(frame, row, col, t, cam, camCenterWorld, zmap);
        }

        for(const auto &a : agents) {
            int approxRow = (int)round(a.renderCell().y);
            int approxCol = (int)round(a.renderCell().x);
            if(approxRow + approxCol == sum) {
                drawHumanDirectionalAt(frame, a.renderCell(), a.facingAngle, a.colorVariant, a.maleStyle, cam, camCenterWorld, zmap);
            }
        }

        int prow = (int)round(player.renderCell().y);
        int pcol = (int)round(player.renderCell().x);
        if(prow + pcol == sum) {
...
int main() {
    ...
    while(true) {
        ...
        if(key == '+') cam.changeZoom(CAMERA_ZOOM_STEP);
        if(key == '-') cam.changeZoom(-CAMERA_ZOOM_STEP);
        ...
        rectangle(frame, Rect(20, 20, 1260, 175), Scalar(0,0,0), FILLED);
        rectangle(frame, Rect(20, 20, 1260, 175), Scalar(95,95,95), 1);
        ...
        ostringstream pss;
        pss << "WASD mueve al jugador | Numpad +/- zoom | Player libre, sin IA | fallback camara en "
